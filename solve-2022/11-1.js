    ...
    constructor(monkeyInput) {
        ...
        for (let line of lines) {
            if (line[0].startsWith('Monkey')) {
                this.id = parseInt(regexGetInteger.exec(line[0])[1]);
            } else if (line[0] === 'Starting items') {
                this.itemsWorryLevel = line[1].split(',').map(v => parseInt(v.trim()));
            } else if (line[0] === 'Operation') {
                this.operation = line[1].split('=')[1].trim().replaceAll('old', 'v');
            } else if (line[0] === 'Test') {
                this.divisibleBy = parseInt(regexGetInteger.exec(line[1])[1]);
            } else if (line[0] === 'If true') {
                this.nextTrue = parseInt(regexGetInteger.exec(line[1])[1]);
            } else if (line[0] === 'If false') {
                this.nextFalse = parseInt(regexGetInteger.exec(line[1])[1]);
            }
        }
