        ...
        if (line[0] === '$') {
            if (line[1] === 'cd') {
                if (line[2] === '..') {
                    ...
                    if (actualNode !== null) actualNode.childNode.push(node);
                    actualNode = node;
                }
            }
        } else {
            if (line[0] === NODE_TYPE_DIR) {
                // consider that each directory will be traversed with the ls command
            } else {
                // Increase size for all parentNode
                let node = actualNode;
                while (node !== null) {
                    node.size += parseInt(line[0]);
                    node = node.parentNode;
                }
                actualNode.childNode.push(new Node(line[1], NODE_TYPE_FILE, actualNode, parseInt(line[0])));
            }
        }
    }
...
const exploreNode = (node) => {
    ...
    if (node.type === NODE_TYPE_DIR) {
        if (node.size < NODE_SIZE_LIMIT) totalSize += node.size;
        for (let childNode of node.childNode) {
