...
const parseData = (data) => {
    ...
    for (let [index, line] of data.split('\n').entries()) {
        ...
        const sensor = {x: parseInt(matches[0][1]), y: parseInt(matches[1][1])};
        const beacon = {x: parseInt(matches[2][1]), y: parseInt(matches[3][1])};
        ...
        if (sensor.y === TARGET_ROW) {
            busyPositionOnTargetRow.add(parseInt(matches[0][1]));
        }
        if (beacon.y === TARGET_ROW) {
            busyPositionOnTargetRow.add(parseInt(matches[2][1]));
        }
    ...
    const W = Xmax - Xmin + 1;
    const H = Ymax - Ymin + 1;
    return {sensors, busyPositionOnTargetRow, Xmin, Xmax, Ymin, Ymax, W, H, distanceMax};
}
...
const getCoveredPositions = (usefulSensors, Xmin, Xmax) => {
    ...
    for (let i = Xmin; i < Xmax; i++) {
        if (busyPositionOnTargetRow.has(i)) continue;
