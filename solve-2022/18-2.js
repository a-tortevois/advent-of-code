...
const parseData = (data) => {
    ...
    const matrix = Array.from({length: W}, () => {
        return Array.from({length: H}, () => {
            return new Array(D).fill(AIR);
        });
    });
...
const getAxisDimensions = (points) => {
    return ['x', 'y', 'z'].map(axe => {
        return Math.max(...points.map(c => c[axe])) + 1;
    });
}
...
const coordToString = ({x, y, z}) => {
    return `(${x};${y};${z})`;
}

const isOnBorder = ({x, y, z}) => {
...
const BFS = (from, visited) => {
    ...
    while (cubesToVisit.length > 0) {
        ...
        for (const cube of cubesToExplore) {
            if (exploredCubes.has(coordToString(cube))) continue;
            visited.add(coordToString(cube));
            if (isOnBorder(cube)) return 0;
