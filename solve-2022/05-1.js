...
const parseCratesInput = (cratesInput) => {
    ...
    for (let i in cratesInput) {
        for (let match of stackCount) {
...
const parseMovesInput = (movesInput) => {
    ...
    for (const move of movesInput) {
        let [crateCount, from, to] = move.match(/\d+/g);
        moves.push({crateCount: parseInt(crateCount), from: from - 1, to: to - 1});
    }
    return moves;
}
...
const applyMoves = (moves, cratesStack) => {
    for (const {crateCount, from, to} of moves) {
        const stackToMove = cratesStack[from].splice(-crateCount).reverse();
