    ...
    constructor(monkeyInput) {
        ...
        for (let line of lines) {
            if (line[0].startsWith('Monkey')) {
                this._id = parseInt(regexGetInteger.exec(line[0])[1]);
            } else if (line[0] === 'Starting items') {
                this._itemsWorryLevel = line[1].split(',').map(v => parseInt(v.trim()));
            } else if (line[0] === 'Operation') {
                this._operation = line[1].split('=')[1].trim().replaceAll('old', 'v');
            } else if (line[0] === 'Test') {
                this._divisibleBy = parseInt(regexGetInteger.exec(line[1])[1]);
            } else if (line[0] === 'If true') {
                this._nextTrue = parseInt(regexGetInteger.exec(line[1])[1]);
            } else if (line[0] === 'If false') {
                this._nextFalse = parseInt(regexGetInteger.exec(line[1])[1]);
            }
        }
...
const main = () => {
    ...
    for (let monkeyInput of input.split('\n\n')) {
        let monkey = new Monkey(monkeyInput);
