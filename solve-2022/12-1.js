...
const isReachable = (from, to) => { // Use with BFS
...
const getNeighbours = (from) => {
    return [{x: from.x + 1, y: from.y}, {x: from.x - 1, y: from.y}, {x: from.x, y: from.y - 1}, {x: from.x, y: from.y + 1}].filter(c => isValidCoord(c))
}
...
const BFS = (from) => {
    ...
    while (toVisit.length > 0) {
        ...
        const coordsToExplore = getNeighbours(currentCoord).filter(c => isReachable(currentCoord, c)).filter(c => !exploredCoords.has(coordToString(c)));
