...
const getRow = () => {
    return Array.from({length: ROOM_WIDE}, () => '.');
}
    ...
    for (let i = 0; i < grid.length; i++) {
        if (!part || i > fromCoord.y || i < fromCoord.y - part.height + 1) {
            ...
            for (const e in part.rocks) {
                ...
                if (coordElem.y === i) row[coordElem.x] = '@';
            }
...
const runFallingPart = (blocNum, iter, lastHighestRock) => {
    while (blocNum < BLOCK_COUNT) {
        let part = getPart(blocNum % parts.length);
        let rowsToAdd = lastHighestRock - UP_OFFSET - part.height;
