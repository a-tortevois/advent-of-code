...
const parseData = (data) => {
    ...
    return {blizzardsPositions, H, W, expeditionPos, exitPoint, entryPoint};
}

const isExitPoint = ({x, y}) => {
    return (x === exitPoint.x && y === exitPoint.y);
}

const isEntryPoint = ({x, y}) => {
...
const getVonNeumannNeighborhood = ({x, y}) => {
    return [DIRECTIONS['>'], DIRECTIONS['v'], DIRECTIONS['<'], DIRECTIONS['^']].map(v => ({x: x + v.x, y: y + v.y})).filter(v => isValidCoord(v));
}

const getHashOfPositionInTime = ({x, y, t}) => {
    return `${x}:${y}:${t}`;
}
