...
const disintegrateBrick = (grid, bricks, brickIndex) => {
  const [[x1, y1, z1], [x2, y2, z2]] = bricks[brickIndex];

  const grid_2 = JSON.parse(JSON.stringify(grid));
  const bricks_2 = JSON.parse(JSON.stringify(bricks));
  ...
  return fallingBricks(grid_2, bricks_2, brickIndex);
};
...
const main = () => {
  ...
  let count = 0;
  for (let brickIndex = 0; brickIndex < bricks.length; brickIndex++) {
    count += disintegrateBrick(grid, bricks, brickIndex);
  }
  return count;
};
